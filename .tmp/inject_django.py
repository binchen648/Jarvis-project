...
# Sidebar hrefs
c = c.replace('<a href="#" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[#9191A8] hover:bg-white/5 transition"><i class="bi bi-grid-fill"></i> Dashboard</a>',
...
c = c.replace('<a href="#" class="flex items-center gap-3 px-4 py-3 rounded-2xl nav-active"><i class="bi bi-stars" style="color: var(--primary)"></i> Agent</a>',
    '<a href="/chat/" class="flex items-center gap-3 px-4 py-3 rounded-2xl nav-active"><i class="bi bi-stars" style="color: var(--primary)"></i> Agent</a>')
c = c.replace('<a href="#" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[#9191A8] hover:bg-white/5 transition"><i class="bi bi-brain"></i> Memory</a>',
...
c = c.replace('<a href="#" class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[#9191A8] hover:bg-white/5 transition"><i class="bi bi-target"></i> Goals</a>',
...
# Search
old_search = '<input type="text" placeholder="搜索对话..." class="bg-white/5 border border-white/10 rounded-lg px-8 py-2 text-xs focus:outline-none focus:border-[var(--primary)] transition w-48 text-white">'
new_search = '<form method="get" action="/chat/" class="relative" style="display:inline">\n                            <i class="bi bi-search absolute left-3 top-2.5 text-[#5C5C72] text-xs"></i>\n                            <input type="text" name="q" placeholder="搜索对话..." value="{{ query }}" class="bg-white/5 border border-white/10 rounded-lg px-8 py-2 text-xs focus:outline-none focus:border-[var(--primary)] transition w-48 text-white">\n                        </form>'
c = c.replace(old_search, new_search)

# New conversation button
old_btn = '<button class="text-xs px-4 py-2 rounded-lg font-bold shadow-lg hover:scale-105 transition flex items-center gap-2" style="background: var(--primary); color: white;">\n                    <i class="bi bi-plus-lg"></i> 新建对话\n                </button>'
new_btn = '<a href="/chat/new/" class="text-xs px-4 py-2 rounded-lg font-bold shadow-lg hover:scale-105 transition flex items-center gap-2" style="background: var(--primary); color: white;">\n                    <i class="bi bi-plus-lg"></i> 新建对话\n                </a>'
c = c.replace(old_btn, new_btn)

# Cards -> Django loop
static_cards_start = '<!-- 对话卡片 1 -->'
cards_end_marker = '            </div>\n        </div>\n\n        <!-- 悬浮 AI Core -->'

idx_start = c.find(static_cards_start)
idx_end = c.find(cards_end_marker, idx_start)
...
if idx_start >= 0 and idx_end >= 0:
    before = c[:idx_start]
    after = c[idx_end + len(cards_end_marker):]
