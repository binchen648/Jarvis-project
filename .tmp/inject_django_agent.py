...
# 5. Replace static cards with Django template loop
old_cards = '<!-- 对话卡片 1 -->'
card_end = '            </div>\n        </div>'
card_start_idx = c.find(old_cards)
card_end_idx = c.find(card_end, card_start_idx)

if card_start_idx >= 0 and card_end_idx >= 0:
    django_loop = """{% if conversations %}
                ...
                <a href="/chat/{{ conv.pk }}/" class="glass-card p-6 rounded-[2.5rem] group cursor-pointer no-underline text-[inherit] block"{% if forloop.first %} style="border-top: 2px solid var(--primary)"{% endif %}>
                    ...
                    <div class="flex justify-between items-center pt-4 border-t border-white/5 text-[10px] text-[#5C5C72]">
                        ...
                        {% if forloop.first %}<i class="bi bi-arrow-right text-lg group-hover:translate-x-1 transition" style="color: var(--primary)"></i>{% endif %}
                    </div>
                </a>
                {% endfor %}
                {% else %}
    ...
    before_cards = c[:card_start_idx]
    after_cards = c[card_end_idx + len(card_end):]
    c = before_cards + django_loop + after_cards
